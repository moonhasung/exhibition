...
SELECT id, passwd, mname, tel, email, job, mdate, fname, r
FROM(
	SELECT id, passwd, mname, tel, email, job, mdate, fname, rownum as r
	FROM( 
		SELECT id, passwd, mname, tel, email, job, mdate, fname
		FROM member  
		where id like '%user1%' 
		order by mdate DESC
		)
